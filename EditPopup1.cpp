...
System::Void CafeStock::EditPopup1::btnBack_Click(System::Object^ sender, System::EventArgs^ e) {
...
System::Void CafeStock::EditPopup1::editCombo_SelectedIndexChanged(System::Object^ sender, System::EventArgs^ e) {
    // Implementation for when the combo box selection changes
    String^ selectedType = editCombo->SelectedItem->ToString();
    // You can add logic here if needed
}
System::Void CafeStock::EditPopup1::btnAdd_Click(System::Object^ sender, System::EventArgs^ e) {
    ...
    try {
        ...
        if (rowsAffected > 0) {
            ...
            // Find and refresh InventoryMenu
            Form^ inventoryForm = Application::OpenForms["InventoryMenu"];
            if (inventoryForm != nullptr) {
                InventoryMenu^ inventory = dynamic_cast<InventoryMenu^>(inventoryForm);
                if (inventory != nullptr) {
                    inventory->LoadDataFromDatabase();  // Reload inventory
                    inventory->Refresh();  // Update UI immediately
                }
            }
